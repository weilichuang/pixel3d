...
class BasicTextureLightMap extends AbstractTriangleRenderer
{
	//texture
		...
		while (ys <ye )
		{
			...
			while (xs <xe )
			{
				...
				if (zi> buffer[pos])
				{
					...
					if (aT < 255) //需要确定此时混合还是不混合lightmap
					{
						var tr:Int =(textel>> 16 & 0xFF);
						var tg:Int =(textel>> 8 & 0xFF);
						var tb:Int = (textel & 0xFF);
