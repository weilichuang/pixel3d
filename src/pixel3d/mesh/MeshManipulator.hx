...
/*
* 所有对IMesh进行的操作只能用于静态模型
*/
class MeshManipulator
{
	public static function setBufferColor(buffer : MeshBuffer, color : UInt) : Void
	{
	...
	public static function transformBuffer(buffer : MeshBuffer, m : Matrix4) : Void
	{
		var vertices : Vector<Vertex>= buffer.getVertices();
		var len : Int = vertices.length;
		var vertex : Vertex;
	...
	public static function scaleBufferTCoords(buffer : MeshBuffer, factor : Point) : Void
	{
		var vertices : Vector<Vertex>= buffer.getVertices();
		var len : Int = vertices.length;
		for(i in 0...len)
	...
	public static function makeBufferPlanarTextureMapping(buffer : MeshBuffer, resolution : Float = 0.01) : Void
	{
		var plane : Plane3D = new Plane3D();
		var vertices : Vector<Vertex>= buffer.getVertices();
		var indices : Vector<Int>= buffer.getIndices();
	...
	* @param	value
	*/
	public static function translateBuffer(buffer : MeshBuffer, value : Vector3D) : Void
	{
		var vertices : Vector<Vertex>= buffer.getVertices();
		var x : Float = value.x;
		var y : Float = value.y;
	...
	* @param	value
	*/
	public static function scaleBuffer(buffer : MeshBuffer, value : Vector3D) : Void
	{
		var vertices : Vector<Vertex>= buffer.getVertices();
	...
	* @param	value
	*/
	public static function rotateBuffer(buffer : MeshBuffer, value : Vector3D) : Void
	{
		var vertices : Vector<Vertex>= buffer.getVertices();
	...
	* @param	value
	*/
	public static function flipBufferSurfaces(buffer : MeshBuffer) : Void
	{
		var indices : Vector<Int>= buffer.getIndices();
