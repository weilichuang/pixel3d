...
var Boot = {
	...
	checkAuthor : function (){
		var AuthorInfo = this.getCookie("SU");
		scope.AuthorUID = AuthorInfo && AuthorInfo != "" ? AuthorInfo.match(/^([^:]*:){2}(\d{5,11})/)[2] : null;
	...
	loadCss : function(){
		...
		if (this.fixConfig() == true) {
			...
			this.dwCSS({url: scope.$BASECSS + 'tpl/' + scope.theme_id + '/t.css?' + cssVer,id: "themeLink"});
			this.writeDiyCss();
		}
	},
